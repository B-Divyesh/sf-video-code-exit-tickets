...
&lt;/script&gt;</code></pre></section>
  ...
  <section class="paid" id="pricing" aria-labelledby="paid-title"><div><p class="eyebrow">CREATOR KIT · ONE-TIME PURCHASE</p><h2 id="paid-title">Build manifests with fewer mistakes</h2><p>The free extension reads hand-written manifests. The $29 Creator Kit adds a guided manifest builder and JSON download.</p><p class="merchant">Sociobot and Dodo are the merchant of record. A refund revokes the license.</p></div><div class="purchase"><a class="button primary" href="https://api.sociobot.in/api/v1/products/${SLUG}/checkout">Buy Creator Kit — $29</a><button class="link-button" id="restore-toggle" type="button">Have a license? Paste it</button><form id="license-form" hidden><label for="license">License token</label><div><input id="license" name="license" autocomplete="off" required><button type="submit">Verify license</button></div><p id="license-status" role="status"></p></form>${hasPaidLicense() ? '<a class="text-link" href="/creator" data-route>Open manifest builder →</a>' : ''}</div></section>`;
}
...
function creatorPage() {
  if (!hasPaidLicense()) return `<section class="narrow legal"><p class="eyebrow">CREATOR KIT</p><h1>Verify a license to build manifests</h1><p>The guided builder is part of the $29 Creator Kit.</p><a class="button primary" href="/#pricing">View the Creator Kit</a></section>`;
...
function bindLicense() {
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    status.textContent = valid ? 'License verified. The manifest builder is ready.' : 'This license is not active. Check the token or buy the Creator Kit.';
